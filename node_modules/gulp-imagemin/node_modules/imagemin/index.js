...
Imagemin.prototype.run = function (cb) {
	...
	if (!self.streams.length) {
		self.use(Imagemin.gifsicle());
		self.use(Imagemin.jpegtran());
		self.use(Imagemin.pngquant());
		self.use(Imagemin.optipng());
		self.use(Imagemin.svgo());
	}

	self.streams.unshift(self.read(self.src()));

	if (self.dest()) {
		self.streams.push(fs.dest(self.dest()));
	}

	var pipe = combine(self.streams);
	var end = concat(function (files) {
		cb(null, files, pipe);
	});

	pipe.on('data', self.emit.bind(self, 'data'));
	pipe.on('end', self.emit.bind(self, 'end'));
	pipe.on('error', cb);
	pipe.pipe(end);
};
