 ...
 * @version $Id$
 */
var _, _buildCss, _buildCssMap, _buildPaths, _cssDistPath, _cssMapName, _cssPath, _hashLen, _isCombo, _mapPath, _stream, binit, butil, color, config, cssBgMap, e, errrHandler, fs, gulp, gutil, md5, mincss, path, plumber, pushCss, rename;
...
_stream = function(files, cb, cb2) {
  ...
  return gulp.src([files]).pipe(plumber({
    ...
    _source = String(source.contents).replace(cssBgReg, function(str, map) {
      var key, val;
      if (map.indexOf('fonts/') !== -1 || map.indexOf('font/') !== -1) {
        return str;
      } else {
