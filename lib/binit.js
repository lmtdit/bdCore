 ...
 * 项目初始化
 */
var Imagemin, _, butil, color, common, config, errrHandler, fs, gulp, gutil, hashLength, imgmin, mapPath, md5, objMixin, path, rename,
...
exports.cpImg = function(cb) {
  var _imgDistPaht, _imgSrcPath;
  _imgSrcPath = path.join(config.rootPath, config.imgSrcPath);
  _imgDistPaht = path.join(config.rootPath, config.imgDistPath);
  return gulp.src(_imgSrcPath + '**/*.{gif,jpg,png,svg}').pipe(gulp.dest(_imgDistPaht)).on('end', function() {
