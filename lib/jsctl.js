...
_buildJs = function(source, outName, cb) {
  var _content, _devPath, _distname, _exclude, _hash, _jsHash, _oldPath, _source, mangled, outPath;
  _jsHash = {};
  ...
  _content = amdclean.clean({
    code: _source,
    wrap: {
      start: indexOf.call(_exclude, outName) >= 0 ? '' : '(function() {\n',
      end: indexOf.call(_exclude, outName) >= 0 ? '' : '\n}());'
    }
  });
...
jsToDist = (function(superClass) {
  ...
  jsToDist.prototype.noamd = function(cb) {
    ...
    _cb = cb || function() {};
    _srcPath = this.srcPath;
    return fs.readdirSync(_srcPath).forEach(function(v) {
      var _jsFile, _outName, _source;
      _jsFile = path.join(_srcPath, v);
      if (fs.statSync(_jsFile).isFile() && v.indexOf('config.') !== 0 && v.indexOf('.') !== 0) {
        _source = fs.readFileSync(_jsFile, 'utf8');
